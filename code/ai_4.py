...
reduceLR = ReduceLROnPlateau(patience = 4,verbose = 1,factor = 0.5) 
...
for i, (train, validation) in enumerate(kFold.split(X, y.argmax(1))) :
    modelCheck = ModelCheckpoint(f'../model_kf/cv_study{i + 1}.h5',save_best_only=True, verbose=0, monitor = 'val_loss', mode = 'min', save_weights_only=True)
    ...
    history = model.fit(X[train], y[train], 
                        ...
                        verbose=1,
                        batch_size=64,
                        callbacks=[earlyStopping, modelCheck, reduceLR])
